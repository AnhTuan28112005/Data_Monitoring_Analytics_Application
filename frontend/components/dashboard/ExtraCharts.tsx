...
export function FearGreedGauge() {
  ...
  return (
    <Card title="Fear & Greed Index">
      ...
      <div className="text-center text-sm text-text-muted -mt-2">{label}</div>
    </Card>
  );
}
  ...
  useEffect(() => {
    ...
    Promise.all(
      PERF_ASSETS.map(async (a) => {
        try {
          const r = await api.ohlcv(a.asset_class, a.symbol, tf, 90);
...
export function VolatilityRadar() {
  ...
  const data = useMemo(() => {
    ...
    // Use |24h change| as a volatility proxy
    const r = items.map((t) => Math.abs(t.change_24h_pct));
    return { labels: [...labels, labels[0]], r: [...r, r[0]] };
  }, [ticks]);
